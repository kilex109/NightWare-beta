...
import nightware.main.module.impl.render.ViewModel;
...
public class ItemRenderer {
    ...
    private void processSpinRotation(EnumHandSide hand) {
        float angle = (System.currentTimeMillis() / SwingAnimations.spinSpeed.get()) % 360L;
        if (NightWare.getInstance().getModuleManager().getModule(SwingAnimations.class).isEnabled() && SwingAnimations.spin.get() && (!this.mc.player.isSwingInProgress || !SwingAnimations.stopOnHit.get())) {
            if (SwingAnimations.spinHand.is("Левую") && hand != EnumHandSide.LEFT || SwingAnimations.spinHand.is("Правую") && hand != EnumHandSide.RIGHT && !SwingAnimations.spinHand.is("Все")) {
                return;
            }

            GlStateManager.rotate(angle, SwingAnimations.spinMode.is("Вертикальный") ? -angle : 0.0F, SwingAnimations.spinMode.is("Горизонтальный") ? 1.0F : 0.0F, 0.0F);
        }
    }
    ...
    public void renderItemInFirstPerson(AbstractClientPlayer p_187457_1_, float p_187457_2_, float p_187457_3_, EnumHand p_187457_4_, float p_187457_5_, ItemStack p_187457_6_, float p_187457_7_) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_)) {
            ...
            if (p_187457_6_.func_190926_b()) {
                ...
                if (NightWare.getInstance().getModuleManager().getModule(ViewModel.class).isEnabled() && (!ViewModel.onlyAura.get() || AimBot.target != null)) {
                    if (flag1) {
                ...
                if (p_187457_1_.isHandActive() && p_187457_1_.getItemInUseCount() > 0 && p_187457_1_.getActiveHand() == p_187457_4_) {
                    ...
                    int i = flag1 ? 1 : -1;
                    if (p_187457_4_ != EnumHand.MAIN_HAND || !NightWare.getInstance().getModuleManager().getModule(SwingAnimations.class).isEnabled() || SwingAnimations.onlyAura.get() && AimBot.target == null) {
                        GlStateManager.translate((float) i * f, f1, f2);
                        this.transformSideFirstPerson(enumhandside, p_187457_7_);
                        this.transformFirstPerson(enumhandside, p_187457_5_);
                        this.processSpinRotation(enumhandside);
                    } else if (SwingAnimations.mode.get().equals("Плавный")) {
                        equipProgress = MathHelper.sqrt(p_187457_5_);
                        GlStateManager.translate((float) i * 0.56F, -0.52F, -0.72F);
                        GlStateManager.rotate((float) i * (45.0F + MathHelper.sin(p_187457_5_ * p_187457_5_ * 3.1415927F) / 4.0F * -20.0F), 0.0F, 1.0F, 0.0F);
                        GlStateManager.rotate((float) i * MathHelper.sin(equipProgress * 3.1415927F) * -20.0F, 0.0F, 0.0F, 1.0F);
                        GlStateManager.rotate(MathHelper.sin(equipProgress * 3.1415927F) * -SwingAnimations.swipePower.get(), 1.0F, 0.0F, 0.0F);
                        GlStateManager.rotate((float) (i * -45), 0.0F, 1.0F, 0.0F);
                        this.processSpinRotation(enumhandside);
                    } else if (SwingAnimations.mode.get().equals("На себя")) {
                        equipProgress = (float) Math.sin((double) p_187457_5_ * 1.5707963267948966D * 2.0D);
                        GlStateManager.translate((float) i * 0.8F, -0.2F, -1.1F);
                        GlStateManager.rotate((float) (i * 90), 0.0F, 1.0F, 0.0F);
                        GlStateManager.rotate((float) (i * -60), 0.0F, 0.0F, 1.0F);
                        GlStateManager.rotate(-SwingAnimations.angle.get() - SwingAnimations.swipePower.get() * equipProgress, 1.0F, 0.0F, 0.0F);
                        this.processSpinRotation(enumhandside);
                    } else if (SwingAnimations.mode.get().equals("Блок")) {
                        equipProgress = (float) Math.sin((double) p_187457_5_ * 1.5707963267948966D * 2.0D);
                        GlStateManager.translate((float) i * 0.8F, -0.2F, -1.1F);
                        GlStateManager.rotate((float) (i * 90), 0.0F, 1.0F, 0.0F);
                        GlStateManager.rotate((float) (i * -30), 0.0F, 0.0F, 1.0F);
                        GlStateManager.rotate(-SwingAnimations.angle.get() - SwingAnimations.swipePower.get() * equipProgress, 1.0F, 0.0F, 0.0F);
                        this.processSpinRotation(enumhandside);
                    } else if (SwingAnimations.mode.get().equals("Старый блок")) {
                        if (flag1) {
                            ...
                            GlStateManager.translate(0.0F, equipProgress / 3.0F * -0.6F, 0.0F);
                            GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
                            float f4 = MathHelper.sin(swingProgress * swingProgress * 3.1415927F);
                            float f5 = MathHelper.sin(MathHelper.sqrt(swingProgress) * 3.1415927F);
                    ...
                    if (NightWare.getInstance().getModuleManager().getModule(ViewModel.class).isEnabled()) {
