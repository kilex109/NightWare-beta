...
import nightware.main.module.impl.render.Crosshair;
import nightware.main.module.impl.util.BetterTab;
...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if (this.titlesTimer > 0)
        {
            ...
            {
                GlStateManager.pushMatrix();
                Fonts.mntsb24.drawCenteredStringWithShadow(this.displayedTitle, (new ScaledResolution(mc)).getScaledWidth() / 2, (new ScaledResolution(mc)).getScaledHeight() / 2 - 10, -1);
                Fonts.mntsb20.drawCenteredStringWithShadow(this.displayedSubTitle, (new ScaledResolution(mc)).getScaledWidth() / 2, (new ScaledResolution(mc)).getScaledHeight() / 2 + 5, -1);
                GlStateManager.popMatrix();
            }
        ...
        if (NightWare.getInstance().getModuleManager().getModule(BetterTab.class).isEnabled() && BetterTab.animation.get()) {
            this.anim = AnimationMath.fast(this.anim, (float)(this.mc.gameSettings.keyBindPlayerList.isKeyDown() ? 1 : 0), 20.0F);
            this.anim = MathHelper.clamp(this.anim, 0.0F, 1.0F);
            GL11.glPushMatrix();
            GL11.glTranslated((double)((float)scaledresolution.getScaledWidth() / 2.0F), 10.0D, 0.0D);
            GL11.glScaled((double)this.anim, (double)this.anim, (double)this.anim);
            GL11.glTranslated((double)((float)(-scaledresolution.getScaledWidth()) / 2.0F), -10.0D, 0.0D);
            this.overlayPlayerList.updatePlayerList(true);
            this.overlayPlayerList.renderPlayerlist(i, scoreboard, scoreobjective1);
            GL11.glPopMatrix();
        } else if (!this.mc.gameSettings.keyBindPlayerList.isKeyDown() || this.mc.isIntegratedServerRunning() && this.mc.player.connection.getPlayerInfoMap().size() <= 1 && scoreobjective1 == null) {
    ...
    private void renderAttackIndicator(float p_184045_1_, ScaledResolution p_184045_2_)
    {
        ...
        if (gamesettings.thirdPersonView == 0)
        {
            ...
            else
            {
                ...
                if (NightWare.getInstance().getModuleManager().getModule(Crosshair.class).isEnabled()) {
                    this.drawTexturedModalRect(l / 2 - 7, i1 / 2 - 7, 0, 0, 16, 16);
    ...
    protected void renderHotbar(ScaledResolution sr, float partialTicks)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            int k = 91;
            this.zLevel = -90.0F;
            RenderUtility.drawGradientGlow(i - 91.5f, sr.getScaledHeight() - 25.5f, 183, 23, 10, new Color(55, 55, 55, 120).getRGB(), new Color(55, 55, 55, 120).getRGB(), new Color(55, 55, 55, 120).getRGB(), new Color(55, 55, 55, 120).getRGB());
            RenderUtility.drawRoundedRect(i - 91.5f, sr.getScaledHeight() - 25.5f, 183, 23, 5, new Color(55, 55, 55, 120).getRGB());
            RenderUtility.drawGradientGlow(i - 90 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 24, 20, 20, 10, new Color(33, 33, 33, 120).getRGB(), new Color(33, 33, 33, 120).getRGB(), new Color(33, 33, 33, 120).getRGB(), new Color(33, 33, 33, 120).getRGB());
            RenderUtility.drawRoundedRect(i - 90 + entityplayer.inventory.currentItem * 20, sr.getScaledHeight() - 24, 20, 20, 5, new Color(33, 33, 33, 120).getRGB());
