...
public abstract class GuiContainer extends GuiScreen
{
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        {
            if (RenderUtility.isHovered(mouseX, mouseY, w / 2 - (Fonts.mntsb16.getStringWidth("Drop All") / 2 + 3), h - 20, Fonts.mntsb16.getStringWidth("Drop All") + 6, 15)) {
                System.out.println("Clicked");

                TimerUtility cd = new TimerUtility();
                for (int i1 = 0; i1 < mc.player.openContainer.inventorySlots.size(); ++i1) {
                    Slot slotik = mc.player.openContainer.inventorySlots.get(i1);
                    if (slotik.getStack().getItem() == Items.DIAMOND_HOE || slotik.getStack().getItem() == Items.field_190931_a) {
                        continue;
                    }
                    cd.hasReached(2500);
                    mc.playerController.windowClick(mc.player.openContainer.windowId, slotik.slotNumber, 1, ClickType.THROW, mc.player);
                    cd.reset();
                }
            }
