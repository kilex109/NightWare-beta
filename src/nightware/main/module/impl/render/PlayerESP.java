...
public class PlayerESP extends Module {
   public static ModeSetting espMode = new ModeSetting("Режим", "Новый", "Новый", "Старый");
   ...
   public PlayerESP() {
      lineWidth.setVisible(() -> espMode.is("Старый"));
      fill.setVisible(() -> espMode.is("Старый"));
      outline.setVisible(() -> espMode.is("Старый"));
   }

   @EventTarget
   public void or3d(EventRender3D eventRender3D) {
      if (espMode.is("Старый")) {
         for (Entity entity : mc.world.loadedEntityList) {
            if (entity == null) return;
            if (entity == mc.player) continue;
            if (entity instanceof EntityPlayer)
               RenderUtility.drawBoxESP(entity.getEntityBoundingBox(), NightWare.getInstance().getC(0), lineWidth.get(), outline.get(), fill.get(), NightWare.getInstance().getC(0).getAlpha(), NightWare.getInstance().getC(0).getAlpha());
         }
      }
   }
