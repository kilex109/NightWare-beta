...
public class Arrows extends Module {
   NumberSetting sizearrows = new NumberSetting("Размер", 1, 1, 2, 0.1f);
   @EventTarget
   public void onDispay(EventRender2D eventDisplay) {
      for (Entity entity : mc.world.loadedEntityList) {
         if (entity != mc.player && entity instanceof EntityPlayer) {
            ...
            GL11.glTranslated((new ScaledResolution(mc)).getScaledWidth() / 2d + 0.5, (new ScaledResolution(mc)).getScaledHeight() / 2d, 0);
            GL11.glTranslated((Math.cos(Math.toRadians(yaw - 90)) * 20), Math.sin(Math.toRadians(yaw - 90)) * 20, 0);
            GL11.glRotated(yaw, 0, 0, 1);
            RenderUtility.drawTriangle(0,0,0, sizearrows.get(), color.getRGB());
