...
public class IcoButton extends GuiButton {
   public DecelerateAnimation animation;

   public IcoButton(int buttonId, int x, int y, int widthIn, int heightIn, String buttonText, String desc) {
   ...
   public void func_191745_a(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
      if (this.visible) {
         ...
         this.hovered = mouseX >= this.x && mouseY >= this.y && mouseX < this.x + 235 && mouseY < this.y + 25;
         ...
         if (this.hovered) {
            ...
            Fonts.icons21.drawString(this.displayString, this.x + 2, this.y + 2, -1);
         } else {
            RenderUtility.drawFixedGlow(this.x, this.y, width, height, 10, shadawcolar);
            RenderUtility.drawRoundedRect(this.x, this.y, width, height, 5.0F, bgColor);
            Fonts.mntsb16.drawString(this.displayString, this.x + 2, this.y + 2, -1);
         }
      }
   }
}
